...
export function RagManagement() {
  ...
  useEffect(() => {
    ...
    if (isKnownSubject(saved)) setSubject(saved);
    setSubjectReady(true);
  }, []);

  useEffect(() => { if (subjectReady) void refresh(); }, [subject, subjectReady]);
  ...
  const indexSelected = async (list: FileList | null) => {
    ...
    try {
      ...
      const parentCount = result.files.reduce((sum, file) => sum + file.parent_count, 0);
      setNotice(`索引完成：${result.files_indexed} 个文件，${parentCount} 个父块、${result.chunks} 个子块，新增 ${result.added_chunks} 个子块`);
      await refresh();
  ...
  const removeChunk = async (chunk: RagChunk) => {
    ...
    setBusy(true);
    try { await deleteRagChunk(detail.file_id, chunk.chunk_id); setDetail(await getRagIndex(detail.file_id, detail.chunk_offset, detail.chunk_limit)); setFiles((items) => items.map((item) => item.file_id === detail.file_id ? {...item, chunks: Math.max(0, item.chunks - 1), child_count: Math.max(0, item.child_count - 1)} : item)); }
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-100"><header className="border-b border-slate-800"><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-8"><div className="flex items-center gap-3"><Link href="/" className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-cyan-300" aria-label="返回对话"><ArrowLeft size={18}/></Link><div><div className="flex items-center gap-2 text-lg font-semibold"><BookOpen size={19} className="text-cyan-300"/>RAG 管理</div><div className="text-xs text-slate-500">按文件隔离的索引、Chunk 和检索参数</div></div></div><div className="flex items-center gap-2"><select value={subject} disabled={!subjectReady} onChange={(event) => changeSubject(event.target.value)} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm disabled:opacity-60"><option value="calculus">微积分</option><option value="linear_algebra">线性代数</option><option value="physics">大学物理</option><option value="chemistry">化学</option><option value="programming">C / 算法</option></select><Button type="button" variant="ghost" size="icon" onClick={() => void refresh()} disabled={loading || busy || !subjectReady} title="刷新"><RefreshCw size={16} className={loading ? "animate-spin" : ""}/></Button></div></div></header>
    <div className="mx-auto grid max-w-6xl gap-5 px-4 py-6 sm:px-8 lg:grid-cols-[280px_1fr]"><aside className="space-y-4"><section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><h2 className="text-sm font-semibold">RAG 参数</h2><p className="mt-1 text-xs leading-5 text-slate-500">新建索引和对话检索使用以下服务端配置。</p><div className="mt-4 space-y-3"><SettingInput label="子块字符数" value={settings.chunk_chars} min={128} max={8192} onChange={(value) => setSettings({...settings, chunk_chars: value})}/><SettingInput label="初始召回子块 TopK" value={settings.retrieval_top_k} min={4} max={64} onChange={(value) => setSettings({...settings, retrieval_top_k: value})}/><div className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-400"><div className="text-slate-300">Reranker 最终子块：4</div><div className="mt-1 leading-5 text-slate-500">固定取重排前 4 个子块，再映射并去重为完整父块；父块总量不单独设限。</div></div><Button type="button" className="w-full gap-2" onClick={() => void saveSettings()} disabled={busy}><Save size={15}/>保存参数</Button></div></section><section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><h2 className="text-sm font-semibold">添加教材</h2><p className="mt-1 text-xs leading-5 text-slate-500">服务端按小节生成父块，再切分子块并写入 LightRAG 向量索引。</p><input ref={fileInput} type="file" multiple accept={accepted} className="hidden" onChange={(event) => void indexSelected(event.target.files)}/><input ref={folderInput} type="file" multiple className="hidden" {...({webkitdirectory: ""} as Record<string, string>)} onChange={(event) => void indexSelected(event.target.files)}/><div className="mt-3 grid grid-cols-2 gap-2"><Button type="button" variant="secondary" className="gap-2" onClick={() => fileInput.current?.click()} disabled={busy}><FilePlus2 size={15}/>选择文件</Button><Button type="button" variant="secondary" className="gap-2" onClick={() => folderInput.current?.click()} disabled={busy}><FolderOpen size={15}/>选择目录</Button></div></section>{notice && <div className="rounded-xl border border-cyan-900/60 bg-cyan-950/30 px-3 py-2 text-xs leading-5 text-cyan-200">{notice}</div>}</aside><section className="min-w-0"><div className="mb-3 flex items-center justify-between"><div><h1 className="text-xl font-semibold">{subjectLabel} · 文件索引</h1><p className="mt-1 text-xs text-slate-500">共 {files.length} 个文件；每个文件按父块/子块隔离，可单独删除。</p></div>{busy && <Loader2 size={17} className="animate-spin text-cyan-300"/>}</div>{files.length === 0 && !loading ? <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-16 text-center text-sm text-slate-500">当前学科还没有 RAG 文件索引。</div> : <div className="space-y-3">{files.map((file) => <article key={file.file_id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="truncate text-sm font-medium text-slate-100">{file.filename}</h2><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500"><span>{size(file.size_bytes)}</span><span>{file.parent_count} 父块</span><span>{file.child_count || file.chunks} 子块</span><span>子块 {file.chunk_chars} 字符</span><span>{file.embedding_model}</span></div><div className="mt-1 truncate text-[11px] text-slate-600">{file.collection} · {file.file_id}</div></div><div className="flex shrink-0 gap-1"><Button type="button" variant="ghost" size="icon" onClick={() => void openDetail(file)} title="查看父子块"><Eye size={16}/></Button><Button type="button" variant="ghost" size="icon" className="text-rose-300 hover:text-rose-200" onClick={() => void removeFile(file)} title="删除文件"><Trash2 size={16}/></Button></div></div></article>)}</div>}{detail && <div className="fixed inset-0 z-20 bg-slate-950/80 p-4 backdrop-blur-sm sm:p-8"><div className="mx-auto flex h-full max-w-5xl flex-col rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl"><div className="flex items-start justify-between border-b border-slate-800 p-4"><div className="min-w-0"><h2 className="truncate font-semibold">{detail.filename}</h2><div className="mt-1 text-xs text-slate-500">{detail.file_id} · {detail.collection} · SHA256 {detail.content_hash}</div><div className="mt-1 text-xs text-slate-400">解析器 {detail.parser_version} · Embedding {detail.embedding_model}</div></div><button type="button" onClick={() => setDetail(null)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-800" aria-label="关闭"><X size={18}/></button></div><div className="flex-1 overflow-y-auto p-4"><div className="mb-3 grid gap-2 text-xs text-slate-400 sm:grid-cols-3"><div>父块数量：{detail.parent_count}</div><div>子块数量：{detail.child_count || detail.chunks}</div><div>子块字符数：{detail.chunk_chars}</div></div><div className="space-y-3">{detail.chunk_items.map((chunk) => <div key={chunk.chunk_id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="flex items-start justify-between gap-3"><div className="text-[11px] text-cyan-300">子块 #{chunk.chunk_index ?? "?"} · {chunk.child_id || chunk.chunk_id} · 父块 {chunk.parent_id || "?"} · {chunk.child_chars || chunk.text.length} 字符</div><Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-rose-300" onClick={() => void removeChunk(chunk)} title="删除子块"><Trash2 size={14}/></Button></div><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-300">{chunk.text}</p>{chunk.parent_text && <details className="mt-2"><summary className="cursor-pointer text-[11px] text-cyan-300">查看完整父块（{chunk.parent_chars} 字符）</summary><p className="mt-2 whitespace-pre-wrap rounded-lg bg-slate-900 p-2 text-xs leading-5 text-slate-400">{chunk.parent_text}</p></details>}<div className="mt-2 text-[10px] text-slate-600">content_hash: {chunk.content_hash} · parser: {chunk.parser_version}</div></div>)}{detail.chunk_items.length === 0 && <div className="py-10 text-center text-sm text-slate-500">没有可显示的子块。</div>}</div></div></div></div>}</section></div></main>;
}
