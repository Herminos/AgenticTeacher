...
import {getModelSettings, resetModelSettings, saveModelSettings} from "@/lib/api";
...
export function ModelSettings({value = initial, onChange}: {value?: ModelConfig; onChange: (next: ModelConfig) => void}) {
  ...
  useEffect(() => {
    let active = true;
    void getModelSettings().then((remote) => {
      if (!active) return;
      onChange({...value, provider: remote.provider, base_url: remote.base_url || "", model: remote.model || undefined, temperature: remote.temperature, api_key_configured: remote.api_key_configured});
    }).catch((reason) => {if (active) setError(reason instanceof Error ? reason.message : "无法读取模型设置");}).finally(() => {if (active) setLoading(false);});
    return () => {active = false;};
  }, [onChange]);
  ...
  const selectProvider = (provider: ProviderId) => {
    ...
    setError("");
    update({provider, base_url: defaults.base_url, model: defaults.model, api_key: "", api_key_configured: false});
  };
  return <div className="relative">
    ...
    {open && <div className="absolute right-0 top-10 z-30 w-[min(92vw,390px)] rounded-xl border border-slate-700 bg-slate-900 p-4 shadow-2xl">
      <div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-medium"><KeyRound size={15} className="text-cyan-300"/>模型 API</div><button type="button" className="text-xs text-cyan-300" onClick={() => {setError(""); void resetModelSettings().then((remote) => {onChange({...initial, provider: remote.provider, base_url: remote.base_url || "", model: remote.model || undefined, temperature: remote.temperature, api_key_configured: remote.api_key_configured}); setSaved(true);}).catch((reason) => setError(reason instanceof Error ? reason.message : "无法恢复默认设置"));}}>恢复 Mock</button></div>
      ...
      {value.provider !== "mock" && <label className="mb-3 block text-xs text-slate-400">个人 API Key / SK{value.api_key_configured && !value.api_key && <span className="ml-2 text-emerald-400">服务端已保存</span>}<input type={visible ? "text" : "password"} value={value.api_key || ""} onChange={(event) => update({api_key: event.target.value, api_key_configured: Boolean(event.target.value)})} placeholder={value.api_key_configured ? "留空以保留已保存密钥" : "输入后点击保存"} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 pr-9 text-sm text-slate-100 outline-none" /> <button type="button" aria-label={visible ? "隐藏密钥" : "显示密钥"} onClick={() => setVisible((current) => !current)} className="relative float-right -mt-8 mr-2 text-slate-500">{visible ? <EyeOff size={15}/> : <Eye size={15}/>}</button></label>}
      ...
      <div className="mb-3 rounded-lg bg-amber-950/30 px-3 py-2 text-[11px] leading-5 text-amber-200/80">设置会保存到后端的本地配置文件；API Key 不会在读取接口中返回明文。留空 Key 后保存会保留已有密钥。</div>
      {error && <div className="mb-3 rounded-lg border border-rose-900/70 bg-rose-950/30 px-3 py-2 text-xs text-rose-200">{error}</div>}
      <Button type="button" size="sm" className="w-full gap-2" disabled={loading} onClick={() => {setError(""); void saveModelSettings(value).then((remote) => {onChange({...value, provider: remote.provider, base_url: remote.base_url || "", model: remote.model || undefined, temperature: remote.temperature, api_key: "", api_key_configured: remote.api_key_configured}); setSaved(true); setTimeout(() => setSaved(false), 1800);}).catch((reason) => setError(reason instanceof Error ? reason.message : "无法保存模型设置"));}}><Save size={14}/>{saved ? "已保存" : loading ? "正在读取…" : "保存模型设置"}</Button>
    </div>}
  </div>;
}
